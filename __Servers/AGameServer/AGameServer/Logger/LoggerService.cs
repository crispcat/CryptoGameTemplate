namespace AGameServer.Logging
{
    ...
    public static class LoggerService
    {
        ...
        private static readonly BufferPool bufferPool = new BufferPool(
            bucketCount: 5, 
            ServerConfig.LG_MIN_MESSAGE_SIZE, 
            ServerConfig.LG_MAX_MESSAGE_SIZE);

        private static int activePort;

        public static void StartListeningForLoggerConnection(int port)
        {
            if (isListening)
                return;
            ...
            isListening = true;
            new Thread(() => SafeInvoke(ListenForLoggerConnections)()).Start();
            
            isSending = true;
            new Thread(() => SafeInvoke(FlushMessagesRoutine)()).Start();
        }
        ...
        private static void ListenForLoggerConnections()
        {
            try
            {
                loggerEndpoint = TcpListener.Create(activePort);
                ...
                while (isListening)
                {
                    Logs.Message($"LoggerService is listening for connections on port {activePort}...");
        ...
        private static void FlushMessagesRoutine()
        {
            ...
            {
                FlushMessages();
                Thread.Sleep(ServerConfig.LG_SEND_PERIOD);
            }
        }
        ...
        private static void FlushMessages()
        {
            try
            {
                ...
                while (
                    processedCount < ServerConfig.LG_MAX_MESSAGES_PER_TICK &&
        ...
        {
            if (message.Length + 1 > ServerConfig.LG_MAX_MESSAGE_SIZE)
